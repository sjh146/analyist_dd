...
The model is fit offline (heavy inference is NOT run in the screener hot loop);
``compute()`` performs a single forward pass over the fitted posterior samples.
"""
import logging
...
class BayesFactorFeatures:
    ...
    def compute(self, close_prices) -> Dict:
        ...
        Returns:
            ...
            0.0 defaults when data is insufficient or numpyro is unavailable.
        """
        ...
        if not _NUMPYRO_AVAILABLE:
            logger.warning("numpyro unavailable; returning default bayes features")
            return dict(defaults)

        try:
            posterior = self._fit(log_rets)
        except Exception as e:
            logger.warning(f"Bayes factor fit failed ({e}); returning defaults")
            return dict(defaults)

        return self._posterior_features(posterior, log_rets)
